...
def main():
    ...
    while True:
        ...
        cv.imshow('Hand Gesture Recognition', debug_image)
            # if hand_sign_label == "Thumbs_Up":
            # increase_brightness_continuously()

    cap.release()
    cv.destroyAllWindows()


def increase_brightness_continuously():
    current_brightness = get_current_brightness()
    while current_brightness < 100:
        set_brightness(current_brightness + 10)
        current_brightness += 10

def decrease_brightness_continuously():
    current_brightness = get_current_brightness()
...
def get_current_brightness():
    result = os.popen("powershell (Get-WmiObject -Namespace root/WMI -Class WmiMonitorBrightness).CurrentBrightness").read()
